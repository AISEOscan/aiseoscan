...
const MAX_SCANS_PER_URL = 2; // Max 2 scans per URL per day
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        ...
        return { 
          ...
          remaining: 0,
          resetAt,
          message: `Rate limit exceeded. You can make ${MAX_REQUESTS_PER_HOUR} requests per hour. Please try again in ${getTimeUntil(resetAt)}.`
        };
      }
    } catch (error) {
